...
Everything here talks to one endpoint on a Laya server you are already running. Start it first
and check it:
...
The bridge is an MCP server in [`../integrations/mcp/laya_mcp.py`](../integrations/mcp/laya_mcp.py):
one file, the official Python SDK, stdio transport.

```bash
pip install "mcp>=2"
# or, to get a `laya-mcp` command on your PATH:
...
| Tool | Answers |
|---|---|
| `laya_check` | a probability that a statement about the state is true |
| `laya_classify` | one of up to 12 labelled options, with the distribution |
| `laya_score` | a position on a 2-10 level scale |
| `laya_gate` | `allow` / `confirm` / `block` for a proposed action, plus the five signals |
| `laya_decide` | any set of questions at once -- one forward pass for all of them |
...
| Variable | Default | |
|---|---|---|
| `LAYA_URL` | `http://localhost:8010` | where the server is |
| `LAYA_API_KEY` | unset | sent as `Authorization: Bearer …` |
| `LAYA_MODEL` | `auto` | or `laya-english`, `laya-multilingual`, `laya-typed-decisions` |
...
```bash
claude mcp add laya --env LAYA_URL=http://localhost:8010 -- python3 /ABS/PATH/integrations/mcp/laya_mcp.py
```
...
* **A skill**, `skills/laya-decisions/SKILL.md`: when to call which tool, how to shape the state
...
|---|---|---|
| `LAYA_GUARD_TIMEOUT` | `3` | seconds to wait for the server |
| `LAYA_GUARD_FAIL_CLOSED` | unset | **fail-open by default**: if the server cannot be reached the hook logs to stderr and stays out of the way. Set to `1` to have it `ask` instead. A decision service that is down should not silently stop your session -- but it should not silently approve either, and that is the switch. |
| `LAYA_GUARD_NO_AUTO_ALLOW` | unset | by default a clean verdict emits `"allow"`, which **skips your normal permission prompt**. Set to `1` and a clean verdict emits nothing instead, so the usual permission flow still runs and the hook can only ever add friction, never remove it. |
| `LAYA_GUARD_LOG` | unset | append one line per decision to this file |

The plugin's [`.mcp.json`](../integrations/claude-code/.mcp.json) points at
`${CLAUDE_PLUGIN_ROOT}/../mcp/laya_mcp.py`, which resolves inside this checkout. If you copy the
plugin directory somewhere else, either copy `integrations/mcp/laya_mcp.py` along with it and
fix the path, or `pip install ./integrations/mcp` and change the command to `laya-mcp`.
...
`laya_gate` before anything destructive.
...
OpenCode's `opencode.json`, Cursor, Windsurf and VS Code. Set Laya up for one of those and omp
...
It needs a runner that can reach the Laya server -- a self-hosted one by default. Set the
repository variable `LAYA_URL` and, if the server wants a key, the secret `LAYA_API_KEY`. The
...
| 3 | the server was unreachable | warn and pass; set the variable `LAYA_FAIL_CLOSED=1` to fail instead |
