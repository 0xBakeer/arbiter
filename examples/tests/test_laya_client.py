...
from laya_client import (
    AuthError, Choice, LayaClient, Noul, OverloadedError, RequestError, Response, Score,
...
def test_every_question_goes_in_one_request():
    with StubServer() as stub:
        LayaClient(base_url=stub.url).system_one("some state", QUESTIONS)
...
def test_accessors_read_the_three_answer_shapes():
    with StubServer() as stub:
        r = LayaClient(base_url=stub.url).system_one("some state", QUESTIONS)
...
def test_the_response_is_the_raw_body():
    with StubServer() as stub:
        r = LayaClient(base_url=stub.url).system_one("some state", {"flag": Noul("x")})
...
def test_a_non_latin_state_is_routed_to_the_multilingual_checkpoint():
    with StubServer() as stub:
        r = LayaClient(base_url=stub.url).system_one("नमस्ते", {"flag": Noul("x")})
    assert r.checkpoint == "multilingual"


def test_label_rejects_a_non_score_answer():
    with StubServer() as stub:
        r = LayaClient(base_url=stub.url).system_one("s", {"flag": Noul("x")})
...
def test_extra_router_hints_are_forwarded():
    with StubServer() as stub:
        LayaClient(base_url=stub.url).system_one("s", {"flag": Noul("x")}, task="invoice",
                                                 lang="de")
...
def test_401_names_the_environment_variable():
    with StubServer(api_key="secret") as stub:
        with pytest.raises(AuthError, match="LAYA_API_KEY"):
            LayaClient(base_url=stub.url).system_one("s", {"flag": Noul("x")})


def test_the_key_is_sent_when_it_is_configured():
    with StubServer(api_key="secret") as stub:
        r = LayaClient(base_url=stub.url, api_key="secret").system_one("s", {"flag": Noul("x")})
...
def test_an_empty_question_set_is_rejected_by_the_server():
    with StubServer() as stub:
        with pytest.raises(RequestError, match="at least one question"):
            LayaClient(base_url=stub.url).system_one("s", {})


def test_529_is_retried_and_then_raised():
    with StubServer(fail="overload") as stub:
        client = LayaClient(base_url=stub.url, retries=2)
...
def test_a_server_that_is_not_there_says_so():
    client = LayaClient(base_url="http://127.0.0.1:1", retries=0, timeout=2)
...
def test_the_environment_supplies_the_defaults(monkeypatch):
    monkeypatch.setenv("LAYA_URL", "http://example.invalid:9999/")
...
def test_explicit_arguments_win_over_the_environment(monkeypatch):
    monkeypatch.setenv("LAYA_URL", "http://example.invalid:9999")
    assert LayaClient(base_url="http://localhost:1234").base_url == "http://localhost:1234"


def test_ready_and_models():
    with StubServer() as stub:
        client = LayaClient(base_url=stub.url)
